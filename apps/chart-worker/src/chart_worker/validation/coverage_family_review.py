...
COVERAGE_FAMILY_REVIEW_VERSION = "coverage-family-review-v1"
OVERLAP_RATIO_MIN = 0.5
ACTIVE_FRAME_RATIO_MIN = 0.5
ACTIVE_ONSET_MIN = 4
LOCAL_STRONG_ATTACK_MIN = 2
...
def review_coverage_family(
    ...
    *,
    ...
    overlapping = tuple(
        sibling for sibling in siblings if _substantially_overlaps(target, sibling)
    )
    model_members = tuple(
        member
        for member in (target, *overlapping)
        if member.model_backed
    )
    fallback_members = tuple(
        member for member in overlapping if not member.model_backed
    )
    key_families = {member.key_mode for member in model_members}
    same_key_siblings = sum(
        member.model_backed and member.key_mode == target.key_mode
    ...
    active = (
        ...
        and audio.active_onset_count >= ACTIVE_ONSET_MIN
        and audio.local_strong_attack_count >= LOCAL_STRONG_ATTACK_MIN
    )
    broadly_shared = (
        len(key_families) >= 2
        and len(model_members) >= 3
    )
    ...
    elif broadly_shared and timing_status is not TimingIntegrityStatus.HEALTHY:
        verdict = CoverageFamilyVerdict.TIMING_AUTHORITY_SUSPECT
        reasons = (
            "ACTIVE_GAP_SHARED_ACROSS_KEY_FAMILIES",
            "TIMING_INTEGRITY_REQUIRES_CORROBORATION",
        )
    elif broadly_shared:
