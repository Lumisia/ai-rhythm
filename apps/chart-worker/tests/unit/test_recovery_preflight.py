...
def _analysis(
    ...
    *,
    active: bool,
    onset_interval_ms: int = 1_000,
) -> OnsetAnalysis:
    onset_ms = tuple(range(0, duration_ms, onset_interval_ms))
    frame_ms = 100
...
def test_slow_bpm_with_many_within_beat_attacks_uses_a_viable_subdivision():
    ...
    assert easy.viable_divisors == (3, 4, 6, 8)


def test_genuinely_sparse_slow_bpm_does_not_trigger_a_fixed_time_gap():
    report = review_recovery_preflight(
        _authority(5.0),
        _analysis(24_000, active=True, onset_interval_ms=12_000),
        duration_ms=24_000,
    )
...
def test_implausibly_slow_grid_with_dense_attacks_is_flagged_as_damaged():
    report = review_recovery_preflight(
        _authority(0.25),
...
def test_preflight_report_keeps_divisor_and_gap_evidence():
    ...
    assert report["difficulties"][0]["activeGaps"]
